...
Token *Scanner::nextToken() {
  ...
  // ID
    ...
    if (lexema == "or")
      return new Token(Token::OR, input, first, current - first);
    if (lexema == "true")
  ...
  // Operadores
    ...
    case ',':
      ...
      break;
    case ':':
      token = new Token(Token::DOSPUNTOS, c);
      break;
    case ';':
    ...
    case '<': {
      if (input[current + 1] == '=') {
        current++;
        token = new Token(Token::MINORQL, input, first, current + 1 - first);
      } else {
        token = new Token(Token::MINOR, c);
      }
      break;
    }
    case '>': {
      if (input[current + 1] == '=') {
        current++;
        token = new Token(Token::GREATEQL, input, first, current + 1 - first);
      } else {
